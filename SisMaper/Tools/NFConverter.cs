...
public class NFConverter
{
    public static void Merge(NF_Result result, NotaFiscal notaFiscal)
    {
        ...
        if (int.TryParse(result.nfe, out var numero)) notaFiscal.Numero = numero;
        if (int.TryParse(result.serie, out var serie)) notaFiscal.Serie = serie;
        using (var client = new WebClient())
        {
            try
            {
                var file = client.DownloadData(notaFiscal.URL_XML);
                if (file != null) notaFiscal.XML = file;
            }
            catch
            {
                //
